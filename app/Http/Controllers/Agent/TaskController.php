...
class TaskController extends Controller
{
    ...
    {
        return view('agent.tasks.create')
    ...
    public function store(SalesCase $salesCase, Request $request)
    {
        ...
        return redirect()->route('agent.task.index', ['salesCase' => $salesCase->id]);
    }

    public function edit(Task $task)
    {
        if ($task->user_id != auth()->id())    abort(401);
        return view('agent.tasks.edit')->with(['task'=> $task]);
    }
    ...
    public function update(Task $task, Request $request)
    {
        ...
        return redirect()->route('agent.task.edit', ['task' => $task->id]);
    }
    ...
    public function destroy(SalesCase $salesCase, Task $task)
    {
        ...
        return redirect()->route('agent.task.index', ['saleCase' => $salesCase->id]);
    }
