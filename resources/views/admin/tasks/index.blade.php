                        ...
                        <table class="table ">
                            ...
                            @foreach($tasks as $key=>$task)
                                <tr>
                                    ...
                                    <td class="text-center align-middle text-nowrap">
                                            <a href="{{route('admin.task.mark-as-done' , ['task' => $task->id])}}"   class="btn btn-icon btn-circle btn-sm btn-outline-success">
                                                <i class="fas fa-check"></i>
                                            </a>
                                            <a href="{{route('admin.sales-case.show' , ['salesCase' => $task->sales_case_id])}}"   class="btn btn-icon btn-circle btn-sm btn-outline-info">
                                                <i class="fas fa-paperclip"></i>
                                            </a>
                                            <a href="{{route('admin.task.edit' , ['task' => $task->id])}}"   class="btn btn-icon btn-circle btn-sm btn-outline-primary">
                                                <i class="fas fa-pencil-alt"></i>
                                            </a>
                                            <form action="{{route('admin.task.destroy' , ['task' => $task->id])}}" method="post" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button
